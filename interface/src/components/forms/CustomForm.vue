        ...
        <div class="row">
            <div v-for="input in inputs" :class="input.class">
                ...
                <div v-else-if="input.type == 'multiselect'">
                    <label :for="`${input.reference}-input`" class="form-label">{{ input.label }}</label>
                    <multiselect v-model="props.form[input.reference]" :options="input.options" :multiple="true"
                        :close-on-select="false" :clear-on-select="false" :preserve-search="true"
                        placeholder="Pesquisar" track-by="label" label="label" deselectLabel="Remover"
                        selectedLabel="Selecionado" selectLabel="Selecionar">
                ...
                <div v-else-if="input.type == 'check'">
                    <input v-model="props.form[input.reference]" class="form-check-input" type="checkbox"
                        :id="`${input.reference}-input`">
                    <label :for="`${input.reference}-input`" class="form-check-label ms-2">{{ input.label }}</label>
                </div>
                <div v-else-if="input.type == 'radio'">
                    <div v-for="option in input.options" :key="option.value" class="form-check">
                        <input class="form-check-input" :value="option.value" type="radio" :name="`name-radio-${input.reference}`" :id="`radio-${option.value}-${input.reference}`" v-model="props.form[input.reference]">
                        <label class="form-check-label" :for="`radio-${option.value}-${input.reference}`">
                            {{ option.label }}
                        </label>
                    </div>
                </div>
                <div v-else-if="input.type == 'textarea'">
                    <label :for="`${input.reference}-input`" class="form-label">{{ input.label }}</label>
                    <textarea v-model="props.form[input.reference]" class="form-control"
                        aria-label="Default select example" />
                </div>
            </div>
        </div>
