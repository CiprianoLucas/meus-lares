<template>
    <div class="d-flex justify-content-between pb-3">
        ...
        <button
            v-if="user?.verified_status == 'pending' || user?.verified_status == 'rejected'"
    ...
    <div v-if="editMode">
        ...
        <text-input
            label="CPF:"
            :validators="[verifyCpf]"
            type="text"
            id="cpf"
            mask="###.###.###-##"
            v-model="documentForm.cpf"
            required
        />
        <text-input
    ...
    <div v-if="['pending', 'rejected'].includes(user.verified_status || '') && !editMode">
        ...
        <file-input
            id="file-document"
            label="Frente do documento de identidade"
            v-model="verificationForm.document_front_photo"
            class="mb-3"
        />
        <file-input
            id="file-document"
            label="Verso do documento de identidade"
            v-model="verificationForm.document_back_photo"
            class="mb-3"
        />
        <file-input
            id="file-document"
            label="Foto do rosto"
            v-model="verificationForm.self_photo"
            class="mb-3"
        />
        <file-input
            id="file-document"
            label="Foto do rosto com documento de identidade ao lado"
            class="mb-3"
...
function startVerification() {
    ...
    app.api
        ...
        .then(() => {
            app.popup('Solicitado!', 'Solicitação de verificação efetuada')
            props.update()
        })
        .catch((error) => {
            editMode.value = true
...
function saveDocuments() {
    ...
    app.api
        ...
        .then(() => {
            app.popup('Atualizado!', 'Documentos atualizados com sucesso')
            props.update()
        })
        .catch((error) => {
            editMode.value = true
