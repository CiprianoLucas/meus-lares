<template>
    <div class="container mt-5">
        ...
        <div class="mb-3">
            ...
            >
                <option v-for="place in places" :key="place.id" :value="place.id">
                    {{ place.name }} - {{ place.street }}, {{ place.number }}, {{ place.city }} -
                    {{ place.state }}
                </option>
            </select>
        </div>
        ...
        <button v-if="!invoiceRelationId" @click="registerInvoiceRelation" class="btn btn-primary">
            Cadastrar
        </button>
        <button v-else @click="updateInvoiceRelation" class="btn btn-primary mx-3">
            Atualizar
        </button>
    </div>
</template>
...
function registerInvoiceRelation() {
    app.api
        .post('/invoice/relation-invoices/', invoiceRelationForm.value)
        .then(({ data }) => {
...
function updateInvoiceRelation() {
    app.api
        .put(`/invoice/relation-invoices/${invoiceRelationId.value}/`, invoiceRelationForm.value)
        .then((response) => {
            sessionStorage.removeItem('/invoice/relation-invoices/')
            app.popup('Sucesso!', 'Relação de fatura salvo', 'success')
            app.redirect('/fatura/relacao-lista')
...
app.onMounted(() => {
    ...
    if (invoiceRelationId.value) {
        app.api
            .get(`/invoice/relation-invoices/${invoiceRelationId.value}/`)
            .then((response) => {
            ...
            .catch(() => {
                app.popup(
                    'Erro!',
                    'Falha ao buscar informações dessa relação de faturas',
                    'warning'
                )
            })
    }
