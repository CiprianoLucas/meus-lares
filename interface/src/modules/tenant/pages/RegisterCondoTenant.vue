<template>
    <div class="container mt-2">
        ...
        <text-input
            ...
            id="tenant-email"
            @change="findUser()"
            v-model="tenantForm.tenant_email"
            required
        />
        <text-input
        ...
        <div v-if="actualTenants !== null && actualTenants.length > 0" class="alert alert-warning">
            ...
            <p class="justify-text">
                Já existe um morador, o novo morador terá vínculo com o atual.
            </p>
            <p class="justify-text">
                Portanto as alterações de contrato, permissões dentre outros aspéctos serão
                diretamente ligadas ao novo morador
            </p>
        </div>
        <radio-input
            help-modal="Teste 123"
            help-tooltip="teste"
            v-else
            v-model="tenantForm.contract"
            label="Tipo de vínculo"
            id="contrato"
            option-value="value"
            option-label="label"
            :options="constractTypes"
...
function findUser() {
    ...
    app.api
        .get('/user/email/' + tenantForm.value.tenant_email)
        .then(({ data }) => {
...
function listTenants() {
    ...
    app.loading(true, 'Buscando...')
    app.api
        .getListCashed('/relation/tenant/?is_active=true&apartment=' + apartmentIdToGet)
        .then(({ result }) => {
...
function listApartments() {
    app.loading(true, 'Buscando...')
    app.api
        .getListCashed('/place/apartments-all/' + condominiumId)
        .then(({ result }) => {
...
function registerTenant() {
    ...
    app.api
        ...
        .then(() => {
            ...
            app.api.clearStartPath(
                '/relation/tenant/?is_active=true&apartment=' + payload.apartment
            )
