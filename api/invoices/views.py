...
from .serializers import InvoicesSerializer, RelationInvoiceSerializer
...
class RelationInvoiceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return RelationInvoice.objects.filter(
            Q(is_active=True) & (Q(place__representative=user) | Q(place__unions=user))
        ).distinct()
...
class InvoicesViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Invoice.objects.filter(
            Q(is_active=True) & Q(relation__resident=user)
        ).distinct()
